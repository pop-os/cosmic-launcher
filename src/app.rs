                    ...
                    pop_launcher::Response::Update(mut list) => {
                        ...
                        if let Some(ref app_name) = self.alt_backtick {
                            if app_name.is_empty() {
                                ...
                                if let Some(first_window) =
                                    list.iter().find(|item| item.window.is_some())
                                {
                                    let app_name_to_filter = first_window.name.clone();
                                    self.alt_backtick = Some(app_name_to_filter.clone());
                                    list.retain(|item| {
                                        item.window.is_some() && item.name == app_name_to_filter
                                    });
                                } else {
                                ...
                                list.retain(|item| {
                                    item.window.is_some() && item.name == app_name_clone
                                });
                            }
